...
const applyStateWideCodes = (countyMap) => {
  ...
  for (const [fips, abbr] of Object.entries(state)) {
    if (fips === "00") {
      continue;
    }
    const target = countyMap[fips] || (countyMap[fips] = {});
    if (!target["000"]) {
      const fullName = stateNames[abbr] || territoryNames[abbr] || abbr;
      target["000"] = `Entire State of ${fullName}`;
    }
  }
};
